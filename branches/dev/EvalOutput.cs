...
namespace Virtual_Lab
{
    ...
    {
        public AudiometerPanel.AudiometerPanelSettings ameterSettings;
        public AudiometerPanel.channelSettings testChannel;
        public AudiometerPanel.channelSettings maskChannel;
    }
    ...
    public class EvalOutput
    {
        ...
        public void CheckMinimum(int frequencyIndex, AudiometerPanel.channelSettings TestChannel, AudiometerPanel.channelSettings MaskChannel)
        {
            if (MaskChannel.interrupt == false)
                return;

            int ear = MaskChannel.route == Ear.left ? 0 : 1;
            int trans = TestChannel.trans == AudiometerPanel.TransducerType.Bone ? 0 : 1;
                ...
                for (cond = 0; cond <= 1; cond++)
                {
                    ...
                    for (mask = 0; mask <= 1; mask++)
                    {
                        ...
                        for (ear = 0; ear <= 1; ear++)
                        {
                            ...
                            {
                                case AudiometerPanel.TransducerType.Bone:
                                    if (cond != 0)
                            ...
                            if (mask == 1)
                            {
                                ...
                                if (StudentAudiometer[freqindex, cond, mask, ear].maskChannel.stim != AudiometerPanel.StimulusType.NB)
                                    colorCode = ColorCode_Red;
                                ...
                                if (StudentAudiometer[freqindex, cond, mask, ear].maskChannel.trans != AudiometerPanel.TransducerType.Phone)
                                    colorCode = ColorCode_Red;
                            }
        ...
        internal void CompileResults(AudiogramPanel adgrmPanel)
        {
